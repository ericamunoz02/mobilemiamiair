...
(function($) {
	...
	$.fn.Fvalidate = function(settings)
	{
		var $form = $(this),
		formId = $form.attr("id"),
		$fields = $form.find("input, mini, select, textarea").filter(":not(:submit)"),
		...
		function _validate(obj)
		{
			...
			if($input.attr("required")=='yes'){
				...
				if(val=='' || val==undefined){
					$input.setError('Please enter your '+Name); return false;
				}
			}
		...
		var methods = {
			...
			showErrors:	function() {
				if(options.alertErrors){
					var s = 'Following errors have occured:', fid = $(this).attr("id");
